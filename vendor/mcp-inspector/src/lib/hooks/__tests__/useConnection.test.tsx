...
describe('useConnection', () => {
    ...
    test('uses the default config values in makeRequest', async () => {
      ...
      expect(mockClient.request).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          maxTotalTimeout: DEFAULT_INSPECTOR_CONFIG.MCP_REQUEST_MAX_TOTAL_TIMEOUT.value,
          resetTimeoutOnProgress:
            DEFAULT_INSPECTOR_CONFIG.MCP_REQUEST_TIMEOUT_RESET_ON_PROGRESS.value
        })
      );
    });
  ...
  test('throws error when mcpClient is not connected', async () => {
    ...
    await expect(result.current.makeRequest(mockRequest, mockSchema)).rejects.toThrow(
