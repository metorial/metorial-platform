...
const ToolsTab = ({
  ...
  useEffect(() => {
    const params = Object.entries(selectedTool?.inputSchema.properties ?? []).map(
      ([key, value]) => [key, generateDefaultValue(value as JsonSchemaType)]
    );
    setParams(Object.fromEntries(params));
  }, [selectedTool]);

  let autoInitRef = useRef(false);
  useEffect(() => {
                        ...
                        {prop.type === 'boolean' ? (
                          ...
                          <div className="mt-1">
                            <DynamicJsonForm
                              ...
                              value={(params[key] as JsonValue) ?? generateDefaultValue(prop)}
