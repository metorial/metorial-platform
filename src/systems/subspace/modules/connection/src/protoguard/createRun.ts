...
let runInclude = { alert: { include: { instances: true } } } as const;

let findRunForMessage = async (messageOid: bigint) =>
  await db.protoGuardRun.findUnique({
    where: { messageOid },
    include: runInclude
  });
...
export let createProtoGuardRunForMessage = async (d: { messageId: string }) => {
  ...
  try {
    ...
    let failedExistingRun = await findRunForMessage(message.oid);
    ...
    try {
      ...
      let existingRun = await findRunForMessage(message.oid);
      if (existingRun) return existingRun;
