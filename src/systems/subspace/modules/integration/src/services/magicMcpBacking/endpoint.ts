...
class magicMcpEndpointBackingServiceImpl {
  ...
  await withMagicMcpBackingLock(
    [
      `endpoint:${d.tenant.id}:${d.solution.id}:${d.environment.id}:${d.input.id}`,
      ...d.input.servers.map(
        server =>
          `server:${d.tenant.id}:${d.solution.id}:${d.environment.id}:${server.magicMcpServerBackingId}`
      )
    ],
    async () =>
      ...
      let serverBackingsById = new Map(serverBackings.map(backing => [backing.id, backing]));
      ...
      let group = await integrationInstanceGroupService.upsertMagicMcpIntegrationInstanceGroup({
        tenant: d.tenant,
        solution: d.solution,
      ...
      await sessionTemplateProviderService.syncHash({ sessionTemplateId: sessionTemplate.id });
    })
  );
