...
export let SessionEvents = ({ session }: { session: SessionsGetOutput | null }) => {
  ...
  let serverSessionItems = serverSessions.data?.items ?? [];
  let hasError = !!serverSessions.error;
  ...
  return (
    <>
      ...
      {serverSessionItems.length == 0 && !serverSessions.isLoading && (
        <>
          <Spacer height={20} />

          <Callout color="gray">
            {hasError
              ? 'No MCP connection logs available for this session yet.'
              : 'You have not connected to this session yet. Once you create an MCP connection, you will be able to view message logs and errors.'}
          </Callout>
        </>
      )}
    </>
