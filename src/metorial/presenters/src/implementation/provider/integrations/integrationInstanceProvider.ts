...
import type { integrationInstanceProviderInclude } from '@metorial-subspace/module-integration';
...
type RawIntegrationInstanceProvider = SubspacePrisma.IntegrationInstanceProviderGetPayload<{
  include: typeof integrationInstanceProviderInclude;
}>;

let requireCurrentVersion = (provider: RawIntegrationInstanceProvider) => {
  if (!provider.currentVersion) {
    throw new Error(
      `Integration instance provider "${provider.id}" has no current version to present.`
    );
  }
...
export let v1IntegrationInstanceProviderPresenter = Presenter.create(
  ...
  .presenter(async ({ integrationInstanceProvider }, opts) => {
    let version = requireCurrentVersion(integrationInstanceProvider);
    let config =
      version.config ?? integrationInstanceProvider.integrationProvider.currentVersion?.config;
    ...
    return {
      ...
      integration_id: integrationInstanceProvider.integration.id,
      integration_instance_id: integrationInstanceProvider.integrationInstance.id,
      tool_filter: presentToolFilter(version.toolFilter, version.isOverrideToolFilter),
      is_override_tool_filter: version.isOverrideToolFilter,
      provider: v1ProviderPreview(integrationInstanceProvider.integrationProvider.provider),
      integration_provider: await v1IntegrationProviderSnapshot(
        integrationInstanceProvider.integrationProvider,
        version.integrationProviderVersion,
        opts
      ),
      config: config
        ? await v1ProviderConfigPreviewPresenter
            ...
            .run()
        : null,
      auth_config: version.authConfig
        ? await v1ProviderAuthConfigPreviewPresenter
            .present(
              {
                authConfig: {
                  ...version.authConfig,
                  providerId: integrationInstanceProvider.integrationProvider.provider.id
                }
              },
              opts
            )
