...
let getIndexName = (suffix?: string) =>
  [env.service.VOYAGER_INDEX_PREFIX, 'audit', suffix].filter(Boolean).join('_');
...
(async () => {
  while (true) {
    console.log('Attempting to create Auditing source in Voyager...');
    ...
    try {
      ...
      console.log('Successfully created Auditing source in Voyager', source.id);
      voyagerSourceProm.resolve(source);
      return;
    } catch (error) {
      console.error(
        'Failed to create Auditing source in Voyager, retrying in 5 seconds...',
        error
      );
    }
...
export let voyagerIndex = {
  eventDestination: await voyager.index.upsert({
    sourceId: (await voyagerSource).id,
    identifier: getIndexName('event_destination'),
