...
class ServerState<T extends JsonValue> implements ServerStateHandle<T> {
        ...
        if (Array.isArray(target)) {
          ...
          let operation: WireOperation =
            ...
            unwrapped.startsWith(oldValue)
              ? [
                  OpCode.StringAppend,
                  [...path, property as string],
                  unwrapped.slice(oldValue.length)
                ]
      ...
      case 'push':
        return (...values: JsonValue[]) => {
          ...
          let rawValues = values.map(value =>
            cloneJson(unwrapValue(value, this.proxyTargets))
          );
  ...
  private recordArraySet(
    ...
    if (index == null) {
      if (
        Object.is((target as unknown as Record<string | symbol, unknown>)[property], value)
      ) {
        return;
      }
    ...
    if (index > target.length)
      throw new Error(`Cannot create sparse array at ${path.join('.')}`);
