...
import { getExecutionContext, provideExecutionContext } from '@metorial/execution-context';
import { useRequestContext } from '@metorial/hono';
...
export let mcpConnectionHandler = async (
  ...
  if (connectionType == 'websocket') {
    ...
    return opts.upgradeWebSocket(c => ({
      ...
      onMessage: async (e, ws) => {
        ...
        try {
          await provideExecutionContext(executionContext, () => manager.handleMessage(data));
        } catch (e) {
  ...
  if (connectionType == 'streamable_http') {
    ...
    if (c.req.method == 'GET') {
      let { connection, close } = await manager.ensureReceiveConnection();
      requestCloseSignal.addEventListener('abort', close);
