  ...
  let proxyStream = new ReadableStream({
    async pull(controller) {
      try {
        ...
        // Rewrite Subspace endpoint URLs to point back to our proxy so
        // clients POST through us rather than directly to Subspace.
            ...
            (_match, prefix, subspacePostUrl) => {
              try {
                ...
                let proxyUrl = new URL(
                  `/mcp/${sessionId}/${providerDeploymentId}/sse`,
                  mcpProxyBase
                );
                if (connectionToken) proxyUrl.searchParams.set('connection_token', connectionToken);
