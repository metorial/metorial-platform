...
export let v1ToolPresenter = Presenter.create(toolType)
  ...
  .schema(
    v.object({
      ...
      id: v.string({ name: 'id', description: 'Unique tool identifier', examples: ['tool_abc123def456'] }),
      ...
      input_schema: v.nullable(
        v.record(v.any(), { name: 'input_schema', description: 'JSON Schema for input parameters', examples: [{ type: 'object', properties: { repo: { type: 'string' }, title: { type: 'string' } } }] })
      ),
      output_schema: v.nullable(
        v.record(v.any(), { name: 'output_schema', description: 'JSON Schema for output format', examples: [{ type: 'object', properties: { id: { type: 'number' }, url: { type: 'string' } } }] })
      ),
