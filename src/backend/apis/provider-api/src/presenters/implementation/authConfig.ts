import { Presenter } from '@metorial/presenter';
import { v } from '@metorial/validation';
import { authConfigType } from '../types';

export let v1AuthConfigPresenter = Presenter.create(authConfigType)
  .presenter(async ({ authConfig }) => ({
    object: 'provider.auth_config' as const,
    id: authConfig.id,
    is_ephemeral: authConfig.isEphemeral ?? false,
    status: authConfig.status ?? 'pending',
    name: authConfig.name,
    description: authConfig.description,
    metadata: authConfig.metadata,
    provider_id: authConfig.providerId,
    provider_deployment_id: authConfig.providerDeploymentId ?? authConfig.deploymentId,
    provider_auth_method_id: authConfig.providerAuthMethodId ?? authConfig.authMethodId,
    created_at: authConfig.createdAt,
    updated_at: authConfig.updatedAt
  }))
  .schema(
    v.object({
      object: v.literal('provider.auth_config'),
      id: v.string({ name: 'id', description: 'Unique auth config identifier', examples: ['acfg_abc123def456'] }),
      is_ephemeral: v.boolean({ name: 'is_ephemeral', description: 'Whether ephemeral', examples: [false] }),
      status: v.string({ name: 'status', description: 'Auth status (active, pending, expired, error)', examples: ['active', 'pending'] }),
      name: v.nullable(v.string({ name: 'name', description: 'Display name', examples: ['GitHub OAuth Token'] })),
      description: v.nullable(v.string({ name: 'description', description: 'Description', examples: ['OAuth token for GitHub API access'] })),
      metadata: v.nullable(v.record(v.any(), { name: 'metadata', description: 'Custom metadata', examples: [{ scopes: ['repo', 'user'] }] })),
      provider_id: v.string({ name: 'provider_id', description: 'Provider ID', examples: ['pvd_abc123def456'] }),
      provider_deployment_id: v.nullable(
        v.string({ name: 'provider_deployment_id', description: 'Deployment ID', examples: ['dep_abc123def456'] })
      ),
      provider_auth_method_id: v.string({
        name: 'provider_auth_method_id',
        description: 'Auth method ID',
        examples: ['auth_abc123def456']
      }),
      created_at: v.date({ name: 'created_at', description: 'Timestamp when created', examples: [new Date('2024-01-15T10:30:00Z')] }),
      updated_at: v.date({ name: 'updated_at', description: 'Timestamp when last updated', examples: [new Date('2024-06-20T14:45:00Z')] })
    })
  )
  .build();
