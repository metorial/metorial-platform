...
export let v1ProviderPresenter = Presenter.create(providerType)
  ...
  .schema(
    v.object({
      object: v.literal('provider'),
      id: v.string({ name: 'id', description: 'Unique provider identifier', examples: ['pvd_abc123def456'] }),
      name: v.string({ name: 'name', description: 'Display name of the provider', examples: ['GitHub MCP Server'] }),
      description: v.nullable(
        v.string({ name: 'description', description: 'Brief description of the provider', examples: ['MCP server for GitHub API integration'] })
      ),
