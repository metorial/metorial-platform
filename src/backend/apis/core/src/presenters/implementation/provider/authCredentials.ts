...
let authCredentialsScopeSchema = v.object({
  object: v.literal('provider.auth_credentials.scope', { description: "String representing the object's type" }),
  id: v.string({ name: 'id', description: 'Unique scope identifier', examples: ['pars_7wXyZaBcDeFgHjKl'] }),
...
export let v1AuthCredentialsPresenter = Presenter.create(authCredentialsType)
  ...
  .schema(
    v.object({
      object: v.literal('provider.auth_credentials', { description: "String representing the object's type" }),
      id: v.string({ name: 'id', description: 'Unique credentials identifier', examples: ['par_4sTuVwXyZaBcDeFg'] }),
      type: v.literal('oauth'),
      name: v.nullable(v.string({ name: 'name', description: 'Display name', examples: ['GitHub OAuth'] })),
      description: v.nullable(v.string({ name: 'description', description: 'Description', examples: ['OAuth credentials for GitHub API'] })),
      metadata: v.nullable(v.record(v.any(), { name: 'metadata', description: 'Custom key-value pairs for storing additional information', examples: [{ app_name: 'My GitHub App', created_by: 'admin@company.com' }] })),
      ...
      scopes: v.nullable(
        v.array(authCredentialsScopeSchema, { name: 'scopes', description: 'OAuth scopes granted to these credentials' })
      ),
