...
export let v1ProviderConfigVaultPresenter = Presenter.create(configVaultType)
  ...
  .schema(
    v.object({
      object: v.literal('provider.config_vault', { description: "String representing the object's type" }),
      id: v.string({ name: 'id', description: 'Unique config vault identifier', examples: ['pcvt_3bCdEfGhJkLmNpQr'] }),
      ...
      metadata: v.nullable(v.record(v.any(), { name: 'metadata', description: 'Custom key-value pairs for storing additional information', examples: [{ owner: 'platform-team', sensitivity: 'high' }] })),
