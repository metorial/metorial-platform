...
export let v1SessionTemplatePresenter = Presenter.create(sessionTemplateType)
  ...
  .schema(
    v.object({
      object: v.literal('session.template', { description: "String representing the object's type" }),
      id: v.string({ name: 'id', description: 'Unique session template identifier', examples: ['stm_2bCdEfGhJkLmNpQr'] }),
      name: v.string({ name: 'name', description: 'Template name', examples: ['Production Template'] }),
      description: v.nullable(v.string({ name: 'description', description: 'Template description', examples: ['Template for production sessions'] })),
