...
export let v1SessionProviderPresenter = Presenter.create(sessionProviderType)
  ...
  .schema(
    v.object({
      object: v.literal('session.provider', { description: "String representing the object's type" }),
      id: v.string({ name: 'id', description: 'Unique session provider identifier', examples: ['spr_3cDeFgHjKlMnPqRs'] }),
      name: v.nullable(v.string({ name: 'name', description: 'Display name', examples: ['GitHub Provider'] })),
      description: v.nullable(v.string({ name: 'description', description: 'Description', examples: ['Active GitHub provider instance'] })),
      status: v.nullable(v.string({ name: 'status', description: 'Provider status', examples: ['active'] })),
      metadata: v.nullable(v.record(v.any(), { name: 'metadata', description: 'Custom key-value pairs', examples: [{ version: '1.0' }] })),
