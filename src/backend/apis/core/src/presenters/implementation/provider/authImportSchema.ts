...
export let v1ProviderAuthImportSchemaPresenter = Presenter.create(authImportSchemaType)
  .presenter(async ({ schema }) => ({
    ...
    auth_config_schema: schema.authConfigSchema
      ? { type: 'json_schema' as const, schema: schema.authConfigSchema }
      : null,

    auth_config_visibility: schema.authConfigVisibility,
  ...
  .schema(
    v.object({
      ...
      auth_config_visibility: v.literal('encrypted', {
