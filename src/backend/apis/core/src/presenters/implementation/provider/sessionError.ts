...
export let v1SessionErrorPresenter = Presenter.create(sessionErrorType)
  ...
  .schema(
    v.object({
      ...
      type: v.nullable(
        v.string({ name: 'type', description: 'Error type', examples: ['RuntimeError'] })
      ),
      name: v.nullable(
        v.string({ name: 'name', description: 'Error name', examples: ['ConnectionTimeout'] })
      ),
      message: v.nullable(
        v.string({
          name: 'message',
      ...
      stack: v.nullable(
        v.string({
          ...
          examples: ['Error: Connection timed out\n    at Socket.connect...']
        })
      ),
      metadata: v.nullable(
        v.record(v.any(), {
      ...
      session_error_group_id: v.nullable(
        v.string({
