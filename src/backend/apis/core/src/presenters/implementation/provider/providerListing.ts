...
export let v1ProviderListingPresenter = Presenter.create(providerListingType)
  ...
  .schema(
    v.object({
      object: v.literal('provider.listing', { description: "String representing the object's type" }),
      id: v.string({ name: 'id', description: 'Unique listing identifier', examples: ['plg_8kLmNpQrStUvWxYz'] }),
      name: v.string({ name: 'name', description: 'Display name', examples: ['GitHub'] }),
      description: v.nullable(v.string({ name: 'description', description: 'Full description', examples: ['Connect to GitHub repositories, issues, and pull requests'] })),
      slug: v.string({ name: 'slug', description: 'URL-friendly identifier', examples: ['github'] }),
      image_url: v.nullable(
        v.string({ name: 'image_url', description: 'URL of the listing logo/icon', examples: ['https://cdn.metorial.com/images/github.png'] })
      ),
      readme: v.nullable(v.string({ name: 'readme', description: 'README content in markdown', examples: ['# GitHub\n\nConnect to GitHub repositories, issues, and pull requests.'] })),
      skills: v.array(v.string(), { name: 'skills', description: 'Capability tags', examples: [['code-review', 'pull-requests']] }),
      flags: v.object({
      ...
      provider_id: v.string({ name: 'provider_id', description: 'Associated provider ID', examples: ['pro_5gHjKlMnPqRsTuVw'] }),
      categories: v.array(v1CategoryPresenter.schema, { name: 'categories', description: 'Provider categories for organization and filtering' }),
      collections: v.array(v1CollectionPresenter.schema, { name: 'collections', description: 'Provider collections this provider belongs to' }),
      groups: v.array(v1GroupPresenter.schema, { name: 'groups', description: 'User groups with access to this provider' }),
