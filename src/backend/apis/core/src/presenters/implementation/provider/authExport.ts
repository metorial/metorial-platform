...
export let v1AuthExportPresenter = Presenter.create(authExportType)
  ...
  .schema(
    v.object({
      object: v.literal('provider.auth_export', { description: "String representing the object's type" }),
      id: v.string({ name: 'id', description: 'Unique auth export identifier', examples: ['pace_7yZaBcDeFgHjKlMn'] }),
      note: v.string({ name: 'note', description: 'Note explaining the export reason', examples: ['Exported for backup purposes'] }),
      metadata: v.nullable(v.record(v.any(), { name: 'metadata', description: 'Custom key-value pairs for storing additional information', examples: [{ exported_by: 'admin@company.com', reason: 'backup' }] })),
      ...
      value: v.record(v.any(), {
        ...
        examples: [{ access_token: 'gho_xxxxxxxxxxxxxxxxxxxx', refresh_token: 'ghr_xxxxxxxxxxxxxxxxxxxx', token_type: 'bearer', scope: 'repo,read:user' }]
      }),
