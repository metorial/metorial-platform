...
let apiHost = new URL(process.env.API_URL || 'http://localhost:3000').host;
...
export let generateSignature = async (
  ...
  let signedPayload = `${apiHost}.${timestamp}.${payload}`;
  ...
  return `i=${apiHost},t=${timestamp},${scheme}=${signature}`;
};
