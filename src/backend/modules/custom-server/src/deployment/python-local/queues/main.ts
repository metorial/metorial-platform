...
import { createPythonLambdaDeployment, PythonDeployment } from '../deployment';

let Sentry = getSentry();

export let pythonDeployMainQueue = createQueue<{
  lambdaId: string;
  serverVersionData: Omit<ServerVersion, 'oid' | 'createdAt' | 'updatedAt'>;
}>({
  name: 'csrv/python/main',
  jobOpts: {
    attempts: 10
  },
  workerOpts: {
...
export let pythonDeployMainQueueProcessor = pythonDeployMainQueue.process(async data => {
  ...
  let python: PythonDeployment;

  try {
    python = await createPythonLambdaDeployment({
  ...
  try {
    while (true) {
      await delay(2000);
      let status = await python.pollDeploymentStatus();
  ...
  try {
    let { capabilities, oauth, callbacks } = await python.discoverServer();
    ...
    if (oauth.enabled) {
      let config = await providerOauthConfigService.createConfig({
        ...
        implementation: {
          ...
          httpEndpoint: python.httpEndpoint,
