import { AssistantModel, db, ID } from '@metorial/db';
import { LanguageModel } from 'ai';
import { getModelInfo } from './models';
import { Provider } from './provider';

export interface Model {
  _persisted: AssistantModel;
  model: LanguageModel;
  provider: Provider;
  slug: string;
  name: string;
  contextWindow: number;
  inputCostPerMillionTokens: number;
  outputCostPerMillionTokens: number;
}

export let model = async (d: {
  slug: string;
  name: string;
  model: LanguageModel;
  provider: Promise<Provider>;
}) => {
  let provider = await d.provider;

  let info = await getModelInfo(provider.slug, d.slug);

  let inner = {
    slug: d.slug,
    name: d.name,
    contextWindow: info?.limit?.context ?? 50_000,
    inputCostPerMillionTokens: info?.cost?.input ?? 0,
    outputCostPerMillionTokens: info?.cost?.output ?? 0
  };

  let _persisted = await db.assistantModel.upsert({
    where: {
      slug_providerOid: { slug: d.slug, providerOid: provider._persisted.oid }
    },
    update: inner,
    create: {
      id: await ID.generateId('assistantModel'),
      providerOid: provider._persisted.oid,
      ...inner
    }
  });

  return {
    _persisted,
    info,
    model: d.model,
    provider,
    ...inner
  };
};
