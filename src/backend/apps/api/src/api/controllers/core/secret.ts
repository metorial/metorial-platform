...
import { organizationCorePath, organizationGroup } from '../../middleware/organizationGroup';
import { secretPresenter } from '../../presenters';

export let secretGroup = organizationGroup.use(async ctx => {
...
export let secretController = Controller.create(
  ...
  },
  {
    list: organizationGroup
      .get(organizationCorePath('secrets', 'secrets.list'), {
        name: 'List secrets',
    ...
    get: secretGroup
      .get(organizationCorePath('secrets/:secretId', 'secrets.get'), {
        name: 'Get secret',
