...
pub async fn run(
    ...
    // Nested below `.control/dev` (not that directory itself) because
    // `turbo::generate` atomically wipes and replaces its target; `docker::start`
    // writes the selected infrastructure's Compose file directly into
    // `.control/dev`, and later cleanup calls depend on that file surviving.
    let workspace = project.root.join(".control/dev/turbo");
