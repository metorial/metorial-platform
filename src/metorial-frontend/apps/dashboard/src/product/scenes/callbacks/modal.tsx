...
let GRACE_PERIOD_OPTIONS = [
  { id: '0', label: 'Immediately — revoke the previous URL now' },
  { id: '3600', label: '1 hour (recommended)' },
...
let CallbackSecretSetupModalContent = (p: {
  ...
  let consumeReceipt = useConsumeCallbackReceiverPathSecretReceipt();
  let [revealedUrl, setRevealedUrl] = useState<string | null>(null);
  let [graceExpiresAt, setGraceExpiresAt] = useState<Date | null>(null);
  let [gracePeriodSeconds, setGracePeriodSeconds] = useState('3600');
  let [revokedImmediately, setRevokedImmediately] = useState(false);
  ...
  let submit = async () => {
    ...
    setRevealedUrl(secureReceiverUrl(p.receiverUrl, consumption.value));
    let immediate = p.mode === 'rotate' && gracePeriodSeconds === '0';
    setRevokedImmediately(immediate);
    setGraceExpiresAt(immediate ? null : mutation.graceExpiresAt);
    p.onComplete();
  };
    ...
    <>
      <Callout color={revealedUrl ? 'orange' : 'gray'}>
        {revealedUrl
          ...
          : 'Generated callback secrets are masked by default. This action consumes an expiring one-time receipt before revealing the secured URL.'}
      </Callout>
        ...
        <>
          ...
          {graceExpiresAt && (
            <>
              ...
              <Callout color="orange">
                <span>
                  The previous URL remains valid through <RenderDate date={graceExpiresAt} />.
                  Update the provider before that grace period ends.
                </span>
              </Callout>
            </>
          )}
        ...
        <>
          ...
          {p.mode === 'rotate' && (
            <>
              <Spacer height={15} />
              <Select
                label="Keep the previous URL valid for"
                description="Pick how long the provider can keep delivering to the old URL while you update it. Choose immediately if the secret may have been leaked."
                value={gracePeriodSeconds}
                onChange={setGracePeriodSeconds}
                items={GRACE_PERIOD_OPTIONS}
              />
            </>
          )}
        </>
      ...
      <Dialog.Actions>
        ...
        {!revealedUrl && (
          <Button
            type="button"
            loading={
              createSecret.isLoading || rotateSecret.isLoading || consumeReceipt.isLoading
            }
            onClick={submit}
          >
            {action}
...
export let showCallbackSecretSetupModal = (p: {
  ...
  showModal(({ dialogProps, close }) => (
    <Dialog.Wrapper {...dialogProps} width={720}>
      ...
      <Dialog.Description>
        The generated value is available only through one audited, single-use receipt.
      </Dialog.Description>
