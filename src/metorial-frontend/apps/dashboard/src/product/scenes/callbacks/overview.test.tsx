...
describe('callback provider setup instructions', () => {
  it('offers authorized start and continue actions only for eligible GitHub BYO state', () => {
    let base = {
      vendor: 'github',
    ...
    expect(buildCallbackProvisionedAppSetup(base)).toEqual(
      expect.objectContaining({
        manifestActionAvailable: true,
        manifestActionLabel: 'Start GitHub setup'
      })
    );
    expect(
      buildCallbackProvisionedAppSetup({
        ...
        githubManifestStateExpiresAt: new Date('2026-08-15T13:00:00.000Z')
      })
    ).toEqual(
      expect.objectContaining({
        manifestActionAvailable: true,
        manifestActionLabel: 'Continue GitHub setup'
      })
    );
    expect(
      buildCallbackProvisionedAppSetup({
    ...
    ).toEqual(expect.objectContaining({ manifestActionAvailable: false }));
  });

  it('returns vendor-specific safe instructions without secret material', () => {
    let slack = buildCallbackProvisionedAppSetup({
      vendor: 'slack',
    ...
    expect(slack.instructions).toMatch(/Event Subscriptions|Request URL/);
    expect(JSON.stringify(slack)).not.toMatch(/secret[_-]?value|token/i);
  });
});
