...
let mocks = vi.hoisted(() => ({
  ...
  retiringSecretValidUntil: null as Date | null,
  hasActiveSecret: false,
  provisionedApps: [] as any[],
  ...
  revokeAllReceiverPathSecrets: vi.fn(),
...
vi.mock('@metorial/state', () => {
  ...
  return {
        ...
        items: [
          {
            ...
            authConfig: null,
            security: {
              receiverId: 'shr_authorized',
              receiverUrl: 'https://receiver.example/callback',
              pathSecrets: [
                ...(mocks.hasActiveSecret
                  ? [
                      {
                        id: 'secret-active',
                        status: 'active',
                  ...
                  : []),
                ...(mocks.retiringSecretValidUntil
                  ? [
                      {
                        id: 'secret-retiring',
                        status: 'retiring',
                        secretVersion: 2,
                        validFrom: new Date('2026-08-15T08:00:00.000Z'),
                        validUntil: mocks.retiringSecretValidUntil,
                        rotatedAt: new Date('2026-08-18T08:00:00.000Z')
                      }
                    ]
                  : [])
              ],
              provisionedApps: mocks.provisionedApps
            },
    ...
    useRevokeCallbackReceiverPathSecret: () => mutation(mocks.revokeReceiverPathSecret),
    useRevokeAllCallbackReceiverPathSecrets: () =>
      mutation(mocks.revokeAllReceiverPathSecrets),
    useConsumeCallbackReceiverPathSecretReceipt: () =>
      mutation(mocks.consumeReceiverPathSecretReceipt),
    useBeginCallbackGithubManifest: () => mutation(mocks.beginGithubManifest),
...
let changeSelect = async (element: HTMLSelectElement, value: string) => {
  await act(async () => {
...
beforeEach(() => {
  ...
  mocks.retiringSecretValidUntil = null;
  mocks.hasActiveSecret = false;
  mocks.provisionedApps = [];
  ...
  mocks.createReceiverPathSecret.mockReset().mockResolvedValue([
    {
      secretIssuanceReceipt: { id: 'csr_authorized', token: 'receipt-token' },
      graceExpiresAt: null
    },
    null
  ]);
  ...
  mocks.revokeReceiverPathSecret
    .mockReset()
    .mockResolvedValue([{ secret: { id: 'secret-1', status: 'revoked' } }, null]);
  mocks.revokeAllReceiverPathSecrets
    .mockReset()
    .mockResolvedValue([{ revokedCount: 2, secrets: [] }, null]);
  mocks.consumeReceiverPathSecretReceipt
    .mockReset()
    .mockResolvedValue([{ value: 'generated secret' }, null]);
  mocks.beginGithubManifest.mockReset().mockResolvedValue([null, new Error('stopped')]);
...
describe('callback overview production editor', () => {
  ...
  it('requires an explicit attach selection and exposes stable config identity', async () => {
    let container = await render(<CallbackOverview callbackId="clb_authorized" />);
    ...
    expect(mocks.addProviderPanelProps).toMatchObject({
      requireExplicitConfigSelection: true,
      includeConfigVaults: false,
      unavailableCombinations: [
        { providerConfigId: 'pcf_authorized', providerAuthConfigId: null }
      ]
    });
...
describe('callback overview authoritative registration rendering', () => {
  ...
  it('renders and invokes the authorized GitHub BYO manifest action', async () => {
    mocks.provisionedApps = [
      {
        id: 'pta-slack-managed',
        generation: 1,
        vendor: 'slack',
        credentialOwnerType: 'managed',
        status: 'active',
        externalAppId: 'managed-slack-app',
    ...
    let container = await render(<CallbackOverview callbackId="clb_authorized" />);
    expect(container.textContent).toContain('Managed slack app: active.');
    expect(container.textContent).toMatch(/Event Subscriptions|Request URL/);
    let setup = Array.from(container.querySelectorAll('button')).find(
      button => button.textContent === 'Start GitHub setup'
    )!;
    expect(
      Array.from(container.querySelectorAll('button')).filter(button =>
        button.textContent?.includes('GitHub setup')
      )
    ).toHaveLength(1);
    await click(setup);
    expect(mocks.beginGithubManifest).toHaveBeenCalledWith({
      ...
      provisionedTenantAppId: 'pta-github'
    });
  });
});

describe('callback secure URL disclosure', () => {
  it('separates secure setup and retiring-secret warnings with readable inline dates', async () => {
    mocks.hasActiveSecret = true;
    let validUntil = new Date(Date.now() + 86_400_000);
    mocks.retiringSecretValidUntil = validUntil;
    let container = await render(<CallbackOverview callbackId="clb_authorized" />);
    let secureSetup = container.querySelector('[data-box-title="Secure callback setup"]')!;
    let warning = Array.from(secureSetup.querySelectorAll('[data-component="callout"]')).find(
      callout => callout.textContent?.includes('Secret version 2')
    )!;
    let warningGroup = warning.parentElement!;

    expect(secureSetup.previousElementSibling?.getAttribute('data-height')).toBe('15');
    expect(warningGroup.getAttribute('data-component')).toBe('flex');
    expect(warningGroup.getAttribute('direction')).toBe('column');
    expect(warningGroup.getAttribute('gap')).toBe('8');
    expect(warningGroup.previousElementSibling?.getAttribute('data-height')).toBe('10');
    expect(warning.textContent).toContain(
      `Secret version 2 is retiring through ${validUntil.toISOString()}. Its URL is still accepted until then.`
    );
    expect(
      Array.from(warning.querySelectorAll('button')).find(
        button => button.textContent === 'Revoke now'
      )
    ).toBeTruthy();
  });

  it('hides lapsed retiring secrets that verification no longer accepts', async () => {
    mocks.hasActiveSecret = true;
    mocks.retiringSecretValidUntil = new Date(Date.now() - 1000);
    let container = await render(<CallbackOverview callbackId="clb_authorized" />);

    expect(container.textContent).not.toContain('Secret version 2 is retiring');
    expect(
      Array.from(container.querySelectorAll('button')).find(
        button => button.textContent === 'Revoke all secrets'
      )
    ).toBeUndefined();
  });

  it('warns that other secrets stay valid and revokes a retiring secret from its warning', async () => {
    mocks.hasActiveSecret = true;
    mocks.retiringSecretValidUntil = new Date(Date.now() + 3_600_000);
    let container = await render(<CallbackOverview callbackId="clb_authorized" />);
    let revokeNow = Array.from(container.querySelectorAll('button')).find(
      button => button.textContent === 'Revoke now'
    )!;

    await click(revokeNow);
    let confirmCall = vi.mocked(confirm).mock.calls.at(-1)![0];
    expect(confirmCall.title).toBe('Revoke callback URL secret');
    expect(confirmCall.description).toContain('1 other secret is still accepted');
    await act(async () => {
      await confirmCall.onConfirm();
    });

    expect(mocks.revokeReceiverPathSecret).toHaveBeenCalledWith({
      instanceId: 'ins_authorized',
      callbackId: 'clb_authorized',
      callbackInstanceId: 'cbi_authorized',
      secretId: 'secret-retiring'
    });
    expect(mocks.revokeAllReceiverPathSecrets).not.toHaveBeenCalled();
  });

  it('offers revoke-all while retiring secrets exist and revokes everything at once', async () => {
    mocks.hasActiveSecret = true;
    mocks.retiringSecretValidUntil = new Date(Date.now() + 3_600_000);
    let container = await render(<CallbackOverview callbackId="clb_authorized" />);
    let revokeAll = Array.from(container.querySelectorAll('button')).find(
      button => button.textContent === 'Revoke all secrets'
    )!;
    expect(revokeAll).toBeTruthy();

    await click(revokeAll);
    let confirmCall = vi.mocked(confirm).mock.calls.at(-1)![0];
    expect(confirmCall.title).toBe('Revoke all callback URL secrets');
    await act(async () => {
      await confirmCall.onConfirm();
    });

    expect(mocks.revokeAllReceiverPathSecrets).toHaveBeenCalledWith({
      instanceId: 'ins_authorized',
      callbackId: 'clb_authorized',
      callbackInstanceId: 'cbi_authorized'
    });
    expect(mocks.revokeReceiverPathSecret).not.toHaveBeenCalled();
  });
  ...
  it('keeps the rotation grace-period message in one inline text flow', async () => {
    let graceExpiresAt = new Date('2026-08-19T10:37:52.000Z');
    mocks.rotateReceiverPathSecret.mockResolvedValue([
      {
        secretIssuanceReceipt: { id: 'csr_rotated', token: 'rotated-token' },
        graceExpiresAt
      },
      null
    ]);
    ...
    let graceMessage = container.querySelector('time')?.parentElement;
    expect(graceMessage?.tagName).toBe('SPAN');
    expect(graceMessage?.textContent).toContain(
      'The previous URL remains valid through 2026-08-19T10:37:52.000Z. Update the provider before that grace period ends.'
    );
  });

  it('revokes the previous URL immediately when zero grace is selected', async () => {
    mocks.rotateReceiverPathSecret.mockResolvedValue([
      {
        secretIssuanceReceipt: { id: 'csr_rotated', token: 'rotated-token' },
        graceExpiresAt: new Date('2026-08-19T10:37:52.000Z')
      },
      null
    ]);
    showCallbackSecretSetupModal({
    ...
    let container = await render(mocks.capturedModal);
    let grace = container.querySelector(
      'select[aria-label="Keep the previous URL valid for"]'
    ) as HTMLSelectElement;
    await changeSelect(grace, '0');
    let rotate = Array.from(container.querySelectorAll('button')).find(
      button => button.textContent === 'Rotate and reveal once'
    )!;
    ...
    expect(container.textContent).not.toContain('remains valid through');
  });
});
