...
import {
  useCallback,
  useBeginCallbackGithubManifest,
  ...
  useRevokeCallbackReceiverPathSecret
} from '@metorial/state';
import {
  Attributes,
  Badge,
...
import {
  ...
  buildCallbackProvisionedAppSetup,
  ...
  return renderWithLoader({ callback: callbackLoader, instances: instancesLoader, provider })(
    ({ callback, instances, provider }) => {
      ...
      let manualSetupItems = instanceItems
        .filter(instance => shouldShowManualWebhookSetup(instance.triggers))
        .map(instance => ({
          id: instance.id,
...
let CallbackInstanceAttachPanelContent = (p: {
  ...
  return (
    <AddProviderPanelFlow
      ...
      requireExplicitConfigSelection
...
let CallbackInstanceDetails = (p: {
  ...
  let security = p.callbackInstance.security;
  let receiverUrl = security.receiverUrl;
  let activeSecret = security.pathSecrets.find(secret => secret.status === 'active');
  // A retiring secret past its grace window is no longer accepted by webhook
  // verification, so it must not be shown as still valid here.
  let retiringSecrets = security.pathSecrets.filter(
    secret =>
      secret.status === 'retiring' &&
      (!secret.validUntil || new Date(secret.validUntil).getTime() > Date.now())
  );
  let triggers = p.callbackInstance.triggers;
  let isAttached = p.callbackInstance.status === 'attached';
  let revokeSecret = useRevokeCallbackReceiverPathSecret();
  let revokeAllSecrets = useRevokeAllCallbackReceiverPathSecrets();
  let reattachCallbackInstance = useCreateCallbackInstance();
  let beginGithubManifest = useBeginCallbackGithubManifest();

  let startGithubManifest = async (provisionedTenantAppId: string) => {
    let [setup, error] = await beginGithubManifest.mutate({
      instanceId: p.instanceId,
      callbackId: p.callbackId,
  ...
  let requestRotation = () =>
    confirm({
      ...
      description:
        'A new secured URL will be revealed once. You choose how long the current URL stays valid in the next step — or revoke it immediately.',
      confirmText: 'Rotate',
      onConfirm: async () => openSecretSetup('rotate')
    });

  let requestRevoke = (secretId: string) => {
    let otherValidSecrets =
      (activeSecret && activeSecret.id !== secretId ? 1 : 0) +
      retiringSecrets.filter(secret => secret.id !== secretId).length;
    confirm({
      title: 'Revoke callback URL secret',
      description:
        otherValidSecrets > 0
          ? `Requests using this exact URL secret will stop immediately. ${otherValidSecrets} other secret${
              otherValidSecrets === 1 ? ' is' : 's are'
            } still accepted — use "Revoke all secrets" to invalidate every URL at once. This cannot be undone.`
          : 'Requests using this exact URL secret will stop immediately. This cannot be undone.',
      confirmText: 'Revoke',
      onConfirm: async () => {
        let [result] = await revokeSecret.mutate({
          instanceId: p.instanceId,
          callbackId: p.callbackId,
          callbackInstanceId: p.callbackInstance.id,
          secretId
        ...
        if (!result) return;
        toast.success('Callback URL secret revoked');
        p.onRefresh();
      }
    });
  };

  let requestRevokeAll = () =>
    confirm({
      title: 'Revoke all callback URL secrets',
      description:
        'Every URL secret for this instance — active and retiring — will stop being accepted immediately. Deliveries will fail until you create a new secure URL. Use this if a secret may have been leaked. This cannot be undone.',
      confirmText: 'Revoke all',
      onConfirm: async () => {
        let [result] = await revokeAllSecrets.mutate({
          instanceId: p.instanceId,
          callbackId: p.callbackId,
          callbackInstanceId: p.callbackInstance.id
        });
        if (!result) return;
        toast.success(
          `Revoked ${result.revokedCount} callback URL secret${
          ...
          {receiverUrl && (
            <>
              ...
              >
                ...
                <Flex gap={8}>
                  {!activeSecret ? (
                    ...
                    <>
                      ...
                      <Button
                        ...
                        color="red"
                        variant="soft"
                        loading={revokeSecret.isLoading}
                        onClick={() => requestRevoke(activeSecret.id)}
                      >
                        Revoke secret
                      </Button>
                    </>
                  ...
                  {retiringSecrets.length > 0 && (
                    <Button
                      ...
                      color="red"
                      variant="soft"
                      loading={revokeAllSecrets.isLoading}
                ...
                {retiringSecrets.length > 0 ? (
                  <>
                    ...
                    <Flex direction="column" gap={8}>
                      {retiringSecrets.map(secret => (
                        <Callout color="orange" key={secret.id}>
                          <Flex align="center" gap={8}>
                            <span>
                              Secret version {secret.secretVersion} is retiring
                              {secret.validUntil ? (
                                <>
                                  {' through '}
                                  <RenderDate date={secret.validUntil} />
                                </>
                              ) : null}
                              . Its URL is still accepted until then.
                            </span>
                            <Button
                              size="1"
                              color="red"
                              variant="ghost"
                              loading={revokeSecret.isLoading}
                              onClick={() => requestRevoke(secret.id)}
                            >
                              Revoke now
                            </Button>
                          </Flex>
                ...
                <revokeSecret.RenderError />
                <revokeAllSecrets.RenderError />

                {security.provisionedApps.map(app => {
                  let setup = buildCallbackProvisionedAppSetup(app);
                  return (
                    <Callout color="gray" key={app.id}>
                      <Text size="2">
                        {app.credentialOwnerType === 'byo' ? 'BYO' : 'Managed'} {app.vendor}{' '}
                        app: {app.status}. {setup.instructions}
                      </Text>
                      {setup.manifestActionAvailable && setup.manifestActionLabel && (
                        <>
                          <Spacer height={8} />
                          <Button
                            size="1"
                            variant="outline"
                            loading={beginGithubManifest.isLoading}
                            onClick={() => startGithubManifest(app.id)}
                          >
                            {setup.manifestActionLabel}
                          </Button>
                        </>
                      )}
                    </Callout>
                ...
                <beginGithubManifest.RenderError />
              </Box>
              <Spacer height={15} />
            </>
          )}
