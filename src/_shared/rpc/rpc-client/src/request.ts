...
import { internalServerError, isServiceError, ServiceError } from '@mtsrc/error';
...
let isServer = !('window' in globalThis);
...
let requesterInternal: Requester = async call => {
  ...
  let maxTries = isServer ? 6 : 3;
  let retryDelay = isServer ? 20 : 1000;
