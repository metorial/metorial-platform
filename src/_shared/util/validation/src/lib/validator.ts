...
import type { Preprocessor, Transformer, ValidationError, ValidationModifier, ValidationResult, ValidationType, ValidatorOptions } from './types';
  ...
  (opts?: Opts): ValidationType<Type> => ({
    validate: rawValue => {
      try {
        ...
        if (handlerRes.success === false) return handlerRes;
