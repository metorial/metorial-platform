...
let toArrayBufferView = (data: Uint8Array) => new Uint8Array(data);
...
let encryptData = async (secretData: string, password: string) => {
  let salt = crypto.getRandomValues(new Uint8Array(new ArrayBuffer(16)));
  let iv = crypto.getRandomValues(new Uint8Array(new ArrayBuffer(12)));
...
let decryptData = async (encryptedData: string, password: string) => {
  ...
  let salt = toArrayBufferView(encryptedDataBuff.slice(0, 16));
