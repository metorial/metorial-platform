...
impl Backend for LocalBackend {
    ...
    async fn get_public_url(
        ...
        let (bucket, object_key) = key.split_once('/').ok_or_else(|| {
            BackendError::Provider("Object key must include a bucket".to_string())
        })?;

        Ok(format!("/buckets/{bucket}/objects/{object_key}"))
    }
}
...
mod tests {
    ...
    #[tokio::test]
    async fn test_local_backend_public_url() {
        let temp_dir = TempDir::new().unwrap();
        let backend = LocalBackend::new(temp_dir.path().to_path_buf(), "test-bucket".to_string());

        let url = backend
            .get_public_url(
                "logical-bucket/path/to/file.txt",
                3600,
        ...
        assert_eq!(url, "/buckets/logical-bucket/objects/path/to/file.txt");
    }
