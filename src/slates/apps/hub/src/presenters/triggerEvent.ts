...
type PresentedTriggerEvent = TriggerEvent & {
...
export let triggerEventListPresenter = async (event: PresentedTriggerEvent) => ({
...
export let triggerEventPresenter = async (event: PresentedTriggerEvent) => {
  ...
  return {
    ...(await triggerEventListPresenter(event)),
    payload
  };
};
