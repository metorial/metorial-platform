...
export let finalizeWebhookRequest = async (d: {
  ...
  return await db.$transaction(async tx => {
    ...
    await tx.slateTriggerWebhookRequest.updateMany({
      where: { id: d.request.id, capturedRequest: { not: Prisma.DbNull } },
      data: {
        capturedRequestExpiresAt: new Date(
          Math.max(
            owned.capturedRequestExpiresAt?.getTime() ?? 0,
