...
export let discoverSlateQueueProcessor = discoverSlateQueue.process(async data => {
  ...
  return discoverLock.usingLock(String(outerVersion.slateOid), async () => {
    ...
    try {
      ...
      // Old providers have no trigger_groups.list handler; calling it
      // unconditionally fails discovery. Keep it in this same Promise.all
      // so the stack batches it into one Function Bay invocation.
