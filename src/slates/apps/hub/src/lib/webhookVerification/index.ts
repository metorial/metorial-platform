...
export type WebhookAtomicSync = Readonly<{
  ...
  capturedSecrets: Readonly<Record<string, { value: string; version: number }>>;
  replayKeys: readonly string[];
  replayTtlSeconds: number;
...
export let executeExactWebhookPipeline = async (d: {
  ...
  for (let item of authorized) {
    ...
    if (rule.result.type === 'sync_only') {
      ...
      let capturedSecrets: Record<string, { value: string; version: number }> = {};
      ...
      if (rule.verify.type === 'provider') {
        ...
        if (
          Object.entries(capturedSecrets).some(([name, captured]) => {
            let currentVersion = Math.max(
              0,
              ...item.trigger.secrets
            ...
            return (
              ...
              !Number.isInteger(captured.version) ||
              captured.version !== currentVersion + 1
            );
          })
        ) {
          return rejectPipeline('provider_invalid_result');
...
export let createInMemoryWebhookAtomicCommitSeam = (initial?: {
  ...
  capturedSecretVersions?: Record<string, number>;
}) => {
  let replay = new Map<string, { requestHash: string; commitId: string }>();
  let states = new Map(Object.entries(initial?.states ?? {}));
  let capturedSecretVersions = new Map(Object.entries(initial?.capturedSecretVersions ?? {}));
  ...
  let seam: WebhookAtomicCommitSeam = {
    commit: async input => {
      ...
      for (let sync of input.syncs) {
        for (let [name, secret] of Object.entries(sync.capturedSecrets)) {
          let key = `${sync.bindings.receiverTriggerId}:${name}`;
          if ((capturedSecretVersions.get(key) ?? 0) >= secret.version) {
            return { status: 'rejected', code: 'mapped_output_invalid' };
          }
        }
      }
      ...
      for (let sync of input.syncs) {
        for (let [name, secret] of Object.entries(sync.capturedSecrets)) {
          capturedSecretVersions.set(
            `${sync.bindings.receiverTriggerId}:${name}`,
            secret.version
          );
        }
      }
  ...
  return { seam, replay, states, capturedSecretVersions, committed };
};
