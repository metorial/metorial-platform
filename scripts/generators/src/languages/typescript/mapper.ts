...
export let generateMapper = async (
  ...
  return (await format(`export let ${name} = ${_generateMapper(type, typename)};\n\n`)) + '\n';
};

let _generateMapper = (type: IntrospectedType, typename?: string): string => {
  if (type.type === 'object') {
...
let objectMapper = (type: IntrospectedType, typename?: string): string => {
  ...
  let properties = Object.entries(type.properties!)
    .map(([key, value]) => {
      let processedType = _generateMapper(value);
...
let arrayMapper = (type: IntrospectedType, typename?: string): string => {
  let processedType = _generateMapper(type.items![0]);
  if (typename) {
