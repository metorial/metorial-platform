...
import { getEndpoints, type IntrospectedType } from './fetch';
...
if (!outputFolder) outputFolder = await input({ message: 'Output folder' });

outputFolder = path.join(process.cwd(), outputFolder);

let { endpoints, types, controllers } = await getEndpoints(url);

await fs.ensureDir(outputFolder);

let resourcesFolder = `${outputFolder}/src/resources`;

await fs.ensureDir(`${outputFolder}/src`);
await fs.emptyDir(`${outputFolder}/src`);
await fs.emptyDir(`${outputFolder}/src`);
...
let appendTypes = async (i: {
  ...
  await fs.appendFile(i.file, await generateTypescriptMapper(mapperName, typeName, i.object));
...
for (let endpoint of endpoints) {
  for (let path of endpoint.allPaths) {
    ...
    let folder = `${outputFolder}/src/resources/${parts.join('/')}`;
    ...
    if (!seenFiles.has(file)) {
      await fs.appendFile(file, `import { mtMap } from '@metorial/util-resource-mapper';\n\n`);
    }
...
let endpointsDir = `${outputFolder}/src/endpoints`;
