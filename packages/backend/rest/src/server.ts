...
import { Authenticator, Context, RequestFlags } from './types';
                ...
                Sentry.withIsolationScope(() =>
                  Sentry.startSpan(
                    ...
                    async span => {
                      ...
                      let sr = c.env.flags.storeRequests;
                      if (sr) {
                        ...
                        //   eventClientService
                        //     .ingestRequest({
                        //       apiKey: (c.env.token as any).apiKey,
                        //       requestId: c.env.requestId,
                        //       error: response.status == 200 ? null : response.body,
                        //       startedAt,
                        //       apiVersion,
                        ...
                        //         headers: Object.entries(c.req.header()),
                        //         body: {
                        //           type: body?.type ?? 'none',
                        //           data: body?.data ?? {}
                        //         }
          ...
          async () => {
            ...
            let corsHeaders: Record<string, string> = {
              ...
              'access-control-allow-headers':
                'Content-Type, Authorization, Cookies, metorial-version, metorial-portal-session-client-secret, metorial-consumer-session-client-secret, metorial-client-secret, metorial-instance-id, metorial-user-id, metorial-organization-id, metorial-project-id, baggage, sentry-trace, metorial-client',
