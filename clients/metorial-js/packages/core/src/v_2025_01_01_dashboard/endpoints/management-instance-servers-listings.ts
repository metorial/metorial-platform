 ...
 */
export class MetorialManagementInstanceServersListingsEndpoint extends BaseMetorialEndpoint<any> {
  ...
  list(instanceId: string, query?: DashboardInstanceServersListingsListQuery) {
    return this._get({
      path: ['instances', instanceId, 'server-listings'],
  ...
  get(instanceId: string, serverListingId: string) {
    return this._get({
      path: ['instances', instanceId, 'server-listings', serverListingId]
