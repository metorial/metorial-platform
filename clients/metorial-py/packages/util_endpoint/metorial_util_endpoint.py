...
class MetorialEndpointManager:
    def __init__(self, config: Any, api_host: str, get_headers, enable_debug_logging: bool = False):
        self.config = config
    ...
    def _request_and_transform(self, method: str, request: MetorialRequest):
        class Transformer:
            def transform(self_inner, mapper):
                ...
                return mapper.transformFrom(data) if hasattr(mapper, 'transformFrom') else mapper(data)
        return Transformer()
