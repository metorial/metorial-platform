...
def mapManagementInstanceSessionsCreateOutput(data: Dict[str, Any]) -> ManagementInstanceSessionsCreateOutput:
  ...
  server_sessions=[{
      ...
      "created_at": item.get('created_at') and datetime.fromisoformat(item.get('created_at'))
    } for item in data.get('server_sessions', [])],
  usage={
...
def mapManagementInstanceSessionsCreateBody(data: Dict[str, Any]) -> ManagementInstanceSessionsCreateBody:
  return ManagementInstanceSessionsCreateBody(
  server_deployments=[            item for item in data.get('server_deployments', [])]
  )
